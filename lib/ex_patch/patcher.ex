defmodule Patcher do
  ...
  def create_and_patch_file(contents, target, patches, assigns) do
    patch_spec = %{name: "Create #{target}", instructions: ""}

    log(:creating, target, fn ->
      if overwrite?(target, contents, assigns) do
        {updated_code, results} =
          Enum.reduce(patches, {contents, []}, fn patch_spec, {code, results} ->
            {result, updated_code} = patch_code(code, patch_spec)
            {updated_code, [{result, target, patch_spec} | results]}
          end)

        if assigns.dry_run do
          IO.puts(updated_code)
        else
          File.mkdir_p!(Path.dirname(target))
          File.write!(target, updated_code)
        end
  ...
  defp log(action, file, fun) do
    ...
    if skipped_postfix != [] do
      Mix.shell().info(
        [IO.ANSI.cursor_up(), :clear_line, :yellow, prefix, :reset, file] ++ skipped_postfix
      )
    end
